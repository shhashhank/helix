...
| **HELIX-9 · Human Approval System** | 🛠️ in progress | Configurable approval gates — *when* a human must sign off, *who* may approve, and the *SLA*. Approval policy model + evaluation (HELIX-128) and a versioned policy admin API in the registry (HELIX-129) landed — Approval Gate Configuration done; request/decision flow, notifications, and audit log still open |
