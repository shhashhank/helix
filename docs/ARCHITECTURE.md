...
flowchart TB
  ...
  subgraph dat["Data & external"]
    ...
    obs{{OTel Collector → Tempo · Prometheus · Grafana<br/>observability/ compose stack}}
  end
  ...
  orc -->|record approval events| aud
  reg -->|OTel tracer at boot| tel
  orc -->|OTel tracer at boot| tel
  tel -->|OTLP traces · opt-in via env| obs
...
| **HELIX-11 · SaaS Platform** | ✅ done | The user-facing platform, built **API-first** (rendered screens deferred to a UI push). **Auth/Orgs/RBAC:** OIDC sign-in + sessions — `@helix/auth` (HELIX-142); row-level tenant isolation — `@helix/tenancy`, closing cross-tenant gaps in the registry (HELIX-143); RBAC roles + `RolesGuard` (HELIX-144). **Request & run dashboard:** submit→run API (HELIX-145), run dashboard API — overview/status/live SSE (HELIX-146), artifact views (HELIX-147). **Onboarding:** GitHub App connect flow with the credential encrypted in the vault (HELIX-148) + a connection health check (HELIX-149). Real IdP/GitHub/AWS bindings and the rendered UI (incl. the approval inbox) are the deferred bindings (DEFERRED.md) |
| **HELIX-150 · Agent Executor** *(forward scope)* | ✅ done | Replaces the worker's stub with the real per-role agents. `@helix/executor`: a `RoleDispatcher` (HELIX-152) over `runAgent`, an `AgentSpecResolver` + default specs (HELIX-153), a generic role executor with step→step context flow (HELIX-154), planning/review (HELIX-155), sandbox-backed coding/testing (HELIX-156), deployment (HELIX-157), and the config-driven worker wiring (HELIX-158: real Anthropic when `ANTHROPIC_API_KEY` is set, else a scripted offline provider). The agents are wired in behind the workspace/tools/deploy seams those sub-tasks left open |
| **HELIX-159 · Sandbox Tools & Repo Checkout** *(forward scope)* | ✅ done | Fills the executor's workspace seams so the agents **do** real work, not just describe it. Run-scoped workspace + run-id threading so a run's steps share one sandbox (HELIX-161); sandbox-bound coding file-edit tools (HELIX-162) and testing command/test tools (HELIX-163); workspace population — scaffold or checkout — + a change-set diff for the PR (HELIX-164); and the worker wiring that swaps the empty tools + bare temp dir for the sandbox-backed pair (HELIX-165). With a key a run now writes files + runs tests in a sandbox. Real `git clone` (DEFERRED #1) + the container/microVM sandbox backend (DEFERRED #3) + AWS deploy (DEFERRED #4) stay deferred |
| **HELIX-166 · Real GitHub + Secrets/KMS bindings** *(forward scope)* | ✅ done | Binds three deferred seams to their real cloud services so runs act on real repos with real credentials. The live Octokit `GitHubClient` (HELIX-168) + a runnable stdio MCP server (HELIX-169); the live `GithubConnectionVerifier` that mints an installation token for the onboarding health check (HELIX-170); and the AWS vault — `AwsKms` (HELIX-171) + a Secrets Manager-backed record store (HELIX-172). Every adapter is config-gated (real when configured, local stand-in otherwise) and mock-tested offline (injected Octokit / `aws-sdk-client-mock`); heavy SDKs are kept behind subpaths. **Residual (manual / separate):** live smoke tests against a real GitHub App + AWS account, and the connect wizard UI (frontend) |
| **HELIX-173 · Frontend web app (React)** *(forward scope)* | ✅ done | The first user-facing app — a React SPA in `apps/web` (webpack + Jest) over the existing APIs. App scaffold + a typed `fetch` API client + auth context/guard + shell (HELIX-175); dev sign-in screen backed by a guarded `POST /auth/dev-login` (HELIX-176); request submission + a **live SSE run dashboard** consumed via `fetch`+`ReadableStream` so it carries the auth header (HELIX-177); the approval inbox (HELIX-178); and the GitHub connect wizard with the live "Test connection" (HELIX-179). Component tests use a mocked `fetch`, so CI stays offline. Real OIDC redirect (DEFERRED #12) + UX/theming polish remain |
